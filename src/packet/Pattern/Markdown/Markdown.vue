...
// 获取 slot 内容容器（复用选择器）
const getSlotContent = (): Element | null => {
return document.querySelector('.flex-1.w-full.lg\\:pr-10')
}

// 查找 Markdown 容器的通用函数
const findMarkdownContainer = (): HTMLElement | null => {
// 优先查找 .markdown-body
let markdownContainer = document.querySelector('.markdown-body') as HTMLElement | null
if (markdownContainer) return markdownContainer
...
if (slotContent) {
  ...
  if (isMarkdownDoc) {
    // 对于 Markdown.md，查找所有可能的容器，选择包含标题的那个
...
try {
  ...
  if (headerIdsProcessed.value) {
    const headers = markdownContainer.querySelectorAll(HEADER_SELECTOR)
    let allHaveIds = true
...
try {
  ...
  if (headers.length === 0) {
    const allHeaders = markdownContainer.querySelectorAll(HEADER_SELECTOR)
    // 如果没有 h2/h3，至少使用 h1

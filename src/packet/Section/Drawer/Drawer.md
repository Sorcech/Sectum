...
Drawer 抽屉组件是一个从屏幕边缘滑出的面板，用于显示额外的内容或操作。它通常用于设置面板、详情页面或辅助功能。支持从四个方向滑出，提供灵活的布局和动画效果。

## 特性

- 📍 **多方向** - 支持从上下左右四个方向滑出
- 📏 **灵活尺寸** - 支持自定义宽度（左右）和高度（上下）
- 📐 **可配置偏移** - 支持为不同方向设置不同的偏移量
- 📜 **内容滚动** - 支持内容区域滚动
- 🎨 **动画效果** - 流畅的滑入滑出动画
- 🎯 **背景遮罩** - 可选的背景遮罩，增强视觉层次
- 🌙 **深色模式** - 完全支持深色模式
...
使用数字或字符串，所有方向使用相同的偏移量：

```vue
<Drawer :isShow="show" @update:isShow="show = $event" :offset="20">
...
<Drawer 
  :isShow="show" 
  @update:isShow="show = $event" 
  :offset="{ top: 0, right: 20, bottom: 0, left: 20 }"
>
  <div class="p-4">内容</div>
</Drawer>
```
...
<Drawer 
  ...
  :offset="[
    ...
    { position: 'right', offset: 20 },
    { position: 'bottom', offset: 0 },
    { position: 'left', offset: 20 }
  ]"
>
  <div class="p-4">内容</div>
</Drawer>
...
<div class="flex flex-wrap gap-3 mb-6">
  ...
  <btn @click="showOffsetDrawer3 = true" color="primary">多方向偏移</btn>
</div>

<Drawer 
  :isShow="showOffsetDrawer1" 
  @update:isShow="showOffsetDrawer1 = $event" 
  title="默认偏移"
  width="w-96"
>
  <div class="p-4">
    <p>使用默认偏移量，所有方向距离屏幕边缘 0 个单位。</p>
  </div>
</Drawer>

<Drawer 
  :isShow="showOffsetDrawer2" 
  @update:isShow="showOffsetDrawer2 = $event" 
  title="自定义偏移"
  width="w-96"
  :offset="{ right: 20 }"
>
  <div class="p-4">
    <p>自定义右侧偏移量为 20 个单位（5rem）。</p>
  </div>
</Drawer>

<Drawer 
  :isShow="showOffsetDrawer3" 
  @update:isShow="showOffsetDrawer3 = $event" 
  title="多方向偏移"
  width="w-96"
  :offset="{ top: 4, right: 20, bottom: 4, left: 20 }"
>
  <div class="p-4">
    <p>为不同方向设置不同的偏移量。</p>
  </div>
...
| `offset` | `number \| string \| object \| array` | `{ top: 0, right: 0, bottom: 0, left: 0 }` | 偏移量配置，支持统一偏移、对象格式或数组格式 |
...
1. **定位**：抽屉固定定位在屏幕指定位置
   - 上方：`top-{offset} left-{offset} right-{offset}`
   - 右侧：`top-{offset} right-{offset}`（默认距离右边 0 个单位）
   - 下方：`bottom-{offset} left-{offset} right-{offset}`
   - 左侧：`top-{offset} left-{offset}`（默认距离左边 0 个单位）
   
   **偏移量说明**：
   - 偏移量使用 Tailwind CSS 的间距单位（1 单位 = 0.25rem = 4px）
   - 默认值：`{ top: 0, right: 0, bottom: 0, left: 0 }`
   - 可以通过 `offset` 属性自定义各方向的偏移量

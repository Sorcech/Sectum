  ...
  <div :class="containerClasses" ref="selectRef">
    ...
    <div ref="selectContainerRef" :class="containerWidthClasses" :style="triggerStyle" class="relative flex items-center">
      ...
      <template v-if="mode === 'default'">
        <ipt type="text" :readonly="!allowInput" :modelValue="selectValue" :placeholder="placedisabled" :disabled="disabled" :size="size" class="w-full"
         @click="toggleShow" @input="handleInput" @keydown.enter="handleEnter" @blur="handleBlur" />
        <div class="absolute right-2 top-1/2 -translate-y-1/2 flex items-center justify-center opacity-50 pointer-events-none" v-show="!disabled">
          <span v-show="!positionShow"><icn name="angle-down" light xl class="text-base-content"></icn></span>
          <span v-show="positionShow"><icn name="angle-up" light xl class="text-base-content"></icn></span>
        </div>
      </template>
        ...
        <div :class="modeContainerClasses">
          <div :class="triggerBaseClasses" @click.stop="toggleShow">
            <avt
              ...
              :clickable="false"
            />
            <span v-if="selectedUser && props.showPlaceholder" :class="textSizeClasses" class="text-base-content whitespace-nowrap flex-1 min-w-0 truncate pr-8">
              {{ selectedUser.label }}
            </span>
            <span v-else-if="props.showPlaceholder && !selectedUser" :class="textSizeClasses" class="text-base-content/60 whitespace-nowrap pr-8">
      ...
      <template v-else-if="mode === 'project'">
        <div :class="modeContainerClasses">
          <div :class="triggerBaseClasses" @click.stop="toggleShow">
            <icn name="layer-group" light :size="iconSize" class="text-primary flex-shrink-0" />
            <span v-if="selectedProject" :class="textSizeClasses" class="text-base-content whitespace-nowrap flex-1 min-w-0 truncate pr-8">
              {{ selectedProject.label }}
            </span>
            <span v-else :class="textSizeClasses" class="text-base-content/60 whitespace-nowrap pr-8">
      ...
      <template v-if="positionShow && !disabled">
        <!-- 项目模式 - 有数据 -->
        <tst v-if="mode === 'project' && projects.length > 0" name="downward" :class="tstClasses" :style="finalPositionStyle">
          <Menu ref="menuRef" compact shadow rounded :class="[menuClasses, menuWidthClass]" :style="menuStyle">
            <btn clean v-for="(project, index) in projects" :key="getOptionKey(project, index, 'project')" @click.stop="handleSelect(project, index)" 
            :class="['flex items-center gap-2 p-2 w-full', selectedProject?.value === project.value ? 'bg-primary/10' : '']">
              ...
              <icn v-if="selectedProject?.value === project.value" name="circle-check"  light  :size="iconSize"
        ...
        <!-- 项目模式 - 空状态 -->
        <tst v-else-if="mode === 'project' && projects.length === 0"  name="downward" 
          :class="tstClasses" :style="finalPositionStyle">
        ...
        <!-- 默认模式 -->
        <tst v-else-if="mode === 'default'" name="downward" :class="tstClasses" :style="finalPositionStyle">
        ...
        <!-- 头像模式 -->
        <tst v-else-if="mode === 'user'" name="downward" :class="tstClasses" :style="finalPositionStyle">
          <Menu compact shadow rounded :class="[menuClasses, menuWidthClass]" :style="menuStyle">
            <btn clean v-for="(user, index) in (props.options || [])" :key="getOptionKey(user, index, 'user')" @click="handleSelect(user, index)" 
            class="flex items-center gap-2 p-2 w-full">
              <avt :src="user.user" :label="user.label" :icon="user.icon" :size="computedUserSize" :status="user.status" :clickable="false" />
...
const WIDTH_CONFIG = { // 宽度配置
  ...
  menu: {
    user: {
      ...
      withoutPlaceholder: 'w-36 min-w-36'
    },
    project: 'min-w-48',
    default: 'min-w-48'
  }
...
const labelClasses = computed(() => {
  const classes = ['select-none', 'text-base-content']
  if (props.label) {
    if (props.direction === 'row') { // 根据 direction 设置不同的样式
      classes.push('pr-2', 'flex-shrink-0') // 水平布局时，右边距，防止被压缩
    } else {
...
const modeContainerClasses = computed(() => { // 统一的外层容器样式（用于 user 和 project 模式）
  ...
  if (props.mode === 'user') { // 根据模式设置宽度和布局
    // 如果指定了 triggerWidth，使用它；否则根据 showPlaceholder 决定
    if (props.triggerWidth) {
      // triggerWidth 已经在 containerWidthClasses 中处理，这里不需要额外设置
      // 但为了确保容器可见，至少设置最小宽度
      baseClasses.push('min-w-16')
    } else {
...
const handleInput = (e: Event) => { // 处理手动输入
  if (!props.allowInput || isDestroyed.value) return
  try {
    const target = e.target as HTMLInputElement
    const inputValue = target.value || ''
    selectValue.value = inputValue
    // 输入时不立即触发 select 事件，等待用户按回车或失去焦点
  } catch (error) {
    console.warn('Handle input error:', error)
  }
}

const handleEnter = (e: KeyboardEvent) => { // 处理回车键
  if (!props.allowInput || isDestroyed.value) return
  try {
    e.preventDefault()
    const target = e.target as HTMLInputElement
    ...
    if (inputValue) {
      // 触发 select 事件，传递输入的值
      // 注意：不在这里设置 selectValue，让 selected prop 来控制显示
      emit('select', { value: inputValue, label: inputValue })
      emit('update:modelValue', inputValue)
      // 关闭下拉菜单（如果打开的话）
      safeSetPositionShow(false)
    }
  } catch (error) {
    console.warn('Handle enter error:', error)
  }
}

const handleBlur = (e: FocusEvent) => { // 处理失去焦点
  if (!props.allowInput || isDestroyed.value) return
  try {
    // 延迟处理，避免与下拉菜单点击冲突
    setTimeout(() => {
      ...
      if (target && document.activeElement !== target) {
        const inputValue = target.value || ''
        if (inputValue && inputValue !== selectValue.value) {
          selectValue.value = inputValue
          // 触发 select 事件，传递输入的值
          emit('select', { value: inputValue, label: inputValue })
          emit('update:modelValue', inputValue)
    ...
    console.warn('Handle blur error:', error)
  }
}

// ==================== 监听器 ====================
watch(() => props.autoOpen, (newAutoOpen) => {
  if (newAutoOpen && !props.disabled && !positionShow.value) {
    // 延迟打开，确保 DOM 已更新
    nextTick(() => {
      if (!isDestroyed.value && isMounted.value) {
        toggleShow()
...
watch(() => props.selected, (newSelected) => {
  // 当 selected 为空字符串、null 或 undefined 时，清空 selectValue
  if (newSelected === null || newSelected === undefined || newSelected === '') {
    selectValue.value = ''
  } else {
    selectValue.value = newSelected
  }
})

watch(() => props.options, (newOptions) => {
  // 当 options 变化时，需要更新内部状态
  if (props.mode === 'user') {
    // 清空内部缓存，强制重新计算 selectedUser
    internalSelectedUser.value = null
    void selectedUser.value
  }
  // 对于其他模式，displayOptions 会自动更新（因为它直接使用 props.options）
...
onMounted(async () => {
  ...
  try {
    ...
    // 只有当 loadProjects 为 true 且没有提供 projects 时才加载项目列表
    // 如果 loadProjects 为 false，说明由父组件（如 ProjectSelect）负责加载项目列表
    if (props.mode === 'project' && props.loadProjects && (!props.projects || props.projects.length === 0)) {
      projects.value = await loadProjectList(props.projects, props.mode, props.loadProjects)
    } else if (props.mode === 'project' && props.projects && props.projects.length > 0) {
      // 如果提供了 projects，直接使用

...
DatePicker 是一个功能完整的日期选择器组件，支持多种日期格式、时间选择、日期范围选择等高级功能。提供直观的日历界面和灵活的配置选项。
...
<div class="flex flex-wrap items-end gap-4 mb-6">
  <DatePicker size="xs" placeholder="XS 尺寸" />
  <DatePicker size="sm" placeholder="SM 尺寸" />
  <DatePicker size="md" placeholder="MD 尺寸" />
  <DatePicker size="lg" placeholder="LG 尺寸" />
  <DatePicker size="xl" placeholder="XL 尺寸" />
</div>

### 不同样式变体

<div class="flex flex-wrap items-end gap-4 mb-6">
  <DatePicker variant="outline" placeholder="Outline 样式" />
  <DatePicker variant="filled" placeholder="Filled 样式" />
  <DatePicker variant="ghost" placeholder="Ghost 样式" />
</div>

### 不同颜色主题

<div class="flex flex-wrap items-end gap-4 mb-6">
  <DatePicker color="primary" placeholder="Primary 主题" />
  <DatePicker color="secondary" placeholder="Secondary 主题" />
  <DatePicker color="success" placeholder="Success 主题" />
  <DatePicker color="warning" placeholder="Warning 主题" />
  <DatePicker color="error" placeholder="Error 主题" />
</div>

### 图标样式

DatePicker 组件支持自定义图标样式，包括 solid、regular、light、thin、brand 五种样式，默认为 light。

<div class="flex flex-wrap items-end gap-4 mb-6">
  <DatePicker icon-style="solid" placeholder="Solid 样式" label="Solid" />
  <DatePicker icon-style="regular" placeholder="Regular 样式" label="Regular" />
  <DatePicker icon-style="light" placeholder="Light 样式" label="Light" />
  <DatePicker icon-style="thin" placeholder="Thin 样式" label="Thin" />
  <DatePicker icon-style="brand" placeholder="Brand 样式" label="Brand" />
</div>
...
const setToday = () => {
  datePickerRef.value?.setValue(new Date())
}
</script>
```
...
### 1. 日期格式选择
- 根据用户地区选择合适的日期格式
- **标准日期格式**: 使用 `YYYY-MM-DD` 作为标准日期格式（仅日期，不含时间）
- **RFC3339/ISO8601 格式**: 当需要包含时间信息或与后端API交互时，组件会自动输出 RFC3339 (ISO8601) 格式，例如 `1990-01-01T00:00:00.000Z` 或 `1990-01-01T00:00:00+08:00`
- 当 `format` 属性包含时间相关格式（如 `HH`, `mm`, `ss`）或包含 `T` 时，组件会输出 RFC3339 格式
- 当 `format` 仅为日期格式（如 `YYYY-MM-DD`）时，组件输出 Date 对象，由外部组件决定如何处理
- 考虑使用时间选择器时包含时间格式

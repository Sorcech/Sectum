...
// 查找颜色值，优先使用小写 key（用于 light 和 dark），如果不存在则使用 camelize
const hex = computed<string>(() => {
  const colorName = color.value.toLowerCase()
  // 先尝试直接使用小写（适用于 light 和 dark）
  if (colors[colorName]?.[+level.value]) {
    return colors[colorName][+level.value]
  }
  // 再尝试 camelize（适用于其他颜色）
  const camelizedName = camelize(color.value)
  return colors[camelizedName]?.[+level.value] || ''
})
</script>
...
<template>
  <div class="flex flex-col items-center">
    <div 
      class="w-14 h-14 rounded border border-gray-200 dark:border-gray-700" 
      :class="props.color"

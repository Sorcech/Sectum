...
- 🖼️ **Logo 支持** - 可选的 Logo 图标显示，自动设置页面 favicon
- 🔖 **自动 Favicon** - 当提供 `logo-icon` 时，自动将页面 favicon 设置为对应的图标
- 📄 **自动标题** - 当提供 `project-name` 时，自动设置页面标题

## 安装

```typescript
import { Navbar, navbarItems, navbarConfig } from 'sectum'
// 或者单独导入组件
...
使用 `logo-icon` 和 `project-name` 属性添加 Logo 图标和项目名称：
...
**自动设置 Favicon 和页面标题**：当提供了 `logo-icon` 和 `project-name` 属性时，Navbar 组件会自动：
- 将页面 favicon 设置为对应的图标（使用 FontAwesome regular 样式）
- 将页面标题设置为项目名称
- 如果 `logo-icon` 或 `project-name` 发生变化，favicon 和页面标题也会自动更新
...
<script setup lang="ts">
import { Navbar, navbarItems } from 'sectum'
...
// 直接导入配置文件
import navbarItems from '~/packet/Model/Navbar/Navbar'
import { navbarItems as defaultNavbarItems } from '~/packet/Model/Navbar/Navbar'
```
...
Navbar 组件的默认配置位于 `~/packet/Model/Navbar/Navbar.ts`：
...
| `project-name` | `string` | `undefined` | 项目名称，显示在 Logo 图标下方。如果未提供，则使用 `config.project.name`。当提供此属性时，会自动将页面标题设置为项目名称 |
| `logo-icon` | `string` | `undefined` | Logo 图标名称，如果未提供则不显示图标。如果未提供，则使用 `config.project.logoIcon`。当提供此属性时，会自动将页面 favicon 设置为对应的图标（使用 FontAwesome regular 样式） |
...
7. **Logo 图标**：如果项目有品牌 Logo，建议使用 `logo-icon` 属性显示。Navbar 组件会自动将 favicon 设置为对应的图标（使用 FontAwesome regular 样式）
8. **项目名称**：使用 `project-name` 属性设置项目名称，Navbar 组件会自动将页面标题设置为项目名称
...
- **Favicon 自动设置**：当提供了 `logo-icon` 属性时，Navbar 组件会在挂载时和 `logo-icon` 变化时自动设置页面 favicon。favicon 使用 FontAwesome regular 样式（`regular`），如果该样式不存在，会自动回退到其他可用样式
- **页面标题自动设置**：当提供了 `project-name` 属性时，Navbar 组件会在挂载时和 `project-name` 变化时自动设置页面标题
- **配置优先级**：如果同时提供了 `project-name`/`logo-icon` prop 和 `config.project.name`/`config.project.logoIcon`，prop 值会优先使用

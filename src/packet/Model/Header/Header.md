...
**自动设置 Favicon**：当提供了 `logo-icon` 属性时，Header 组件会自动将页面 favicon 设置为对应的图标（使用 FontAwesome regular 样式）。如果 `logo-icon` 发生变化，favicon 也会自动更新。

## 配置首页链接

使用 `home-link` 属性配置项目名称的首页链接。点击项目名称时会导航到该链接：
...
1. **Logo 图标**：如果项目有品牌 Logo，建议使用 `logo-icon` 属性显示。Header 组件会自动将 favicon 设置为对应的图标（使用 FontAwesome regular 样式）
2. **首页链接**：为每个项目配置独立的 `home-link`，确保点击项目名称时导航到正确的首页
3. **导航菜单**：推荐使用 `nav-items` prop 传入导航配置，或通过 `routes` prop 自动提取，支持配置文件导入
4. **导航间距**：使用 `nav-items-gap` 调整导航按钮间距，大屏设备可以使用较大间距（如 `8rem`）
5. **图标按钮**：推荐使用 `icon-buttons` 数组配置多个图标按钮，支持灵活扩展
6. **图标样式**：User 图标默认使用 `light` 样式，GitHub 等品牌图标使用 `brand` 样式
7. **背景透明度**：在大屏设备上，可以设置较低透明度（如 0.8）以获得毛玻璃效果
8. **组件集成**：将主题、暗色模式、语言切换组件作为 props 传入，实现完整的功能集成
9. **响应式**：Header 会自动在移动端显示菜单按钮，导航菜单在移动端自动隐藏（`hidden md:flex`）
...
- 背景颜色会根据暗色模式自动调整
- **首页链接**：`home-link` 支持内部路由和外部链接。内部路由使用 `RouterLink`，外部链接使用 `<a>` 标签
...
- **Favicon 自动设置**：当提供了 `logo-icon` 属性时，Header 组件会在挂载时和 `logo-icon` 变化时自动设置页面 favicon。favicon 使用 FontAwesome regular 样式（`regular`），如果该样式不存在，会自动回退到其他可用样式

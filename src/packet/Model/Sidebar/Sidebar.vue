...
const menuGroups = computed(() => {
  ...
  // 若传入的是完整路由配置（顶层为 /sectum 或 /sectum.cc），则使用其 children 作为菜单分组
  const sectumRoute = props.routes.find((r: any) => r.path === '/sectum' || r.path === '/sectum.cc')
  ...
  // 否则认为传入的就是分组数组（如 Introduce、Element、Section 等）
      ...
      <nav aria-label="Docs navigation">
        ...
        <Menu v-if="props.items && props.items.length > 0" class="w-full bg-base-150">
          ...
          <template v-for="(item, index) in props.items" :key="index">
            <btn v-if="item.path" size="xs" clean variant="transparent" @click="handleItemClick(item.path)"
        ...
        <!-- 使用路由配置（向后兼容） -->
        <Menu v-else-if="menuGroups.length > 0" v-for="(parent, index) in menuGroups" :key="parent.meta?.title || parent.path || index" 
          ...
          <template v-if="parent.children && parent.children.length > 0">
            <btn size="xs" clean variant="transparent"  v-for="(child, childIndex) in parent.children"  :key="child.path || childIndex" 
              :class="['block w-full text-left', route?.meta?.title === child.meta?.title ? '!border-l-2 !border-y-0 !border-r-0 !border-solid !border-primary bg-base-100' : '!border-l-2 !border-y-0 !border-r-0 !border-transparent']"
              @click="handleItemClick(child.path)"> {{ child.meta?.title }}</btn>
          </template>
        </Menu>
      </nav>
      </div>
